...
def plot_connectivity_bipartite(con, labels, strength_perc, edge_cmp, node_cmap, fig_title='',
                                only_lbl=None, arrange='network'):
    import itertools
    from tools_general import plot_colorbar2

    edge_thresh = np.percentile(con.ravel(), strength_perc)
    ...
    edge_cmap_range = np.arange(edge_thresh, np.max(con), step=0.001)
    cmap_edge = plt.get_cmap(edge_cmp, len(edge_cmap_range))

    con_deg1 = np.sum(con_lbl_sorted, axis=1)
    con_deg2 = np.sum(con_lbl_sorted, axis=0)
    # cmap_deg1 = plt.get_cmap('YlGnBu', len(con_deg1))  # YlGnBu, Purples
    # cmap_deg2 = plt.get_cmap('pink', len(con_deg2))  # YlGn
    # cmap_deg2 = cmap_deg2.reversed()

    node_cmap_range = np.arange(0, np.max(np.append(con_deg1, con_deg2)), 0.001)
    cmap_node = plt.get_cmap('pink', len(node_cmap_range))
    cmap_node = cmap_node.reversed()
    cmap_deg2, cmap_deg1 = cmap_node, cmap_node

    node_colors_deg1 = [cmap_deg1(ii) for ii in np.argsort(np.argsort(con_deg1))]
    node_colors_deg2 = [cmap_deg2(ii) for ii in np.argsort(np.argsort(con_deg2))]
    ...
    # plot the edges ---------
    ax1 = plt.subplot2grid((8, 3), (0, 0), colspan=3, rowspan=7)
    ...
    # plot the nodes for degree ----------
    for i_lbl, nodes_x in enumerate(nodes_x_start):
        this_node_color = node_colors_deg1[i_lbl]  # cmap_node(np.argmin(np.abs(node_cmap_range - con_deg1[i_lbl])))
        verts = _build_node_verts_bipartite(nodes_x, 1, xw, yw)
        _plot_a_node_bipartite(verts, ax1, this_node_color)

    for i_lbl, nodes_x in enumerate(nodes_x_start):
        this_node_color = node_colors_deg2[i_lbl]  # cmap_node(np.argmin(np.abs(node_cmap_range - con_deg2[i_lbl])))
        verts = _build_node_verts_bipartite(nodes_x, -1, xw, yw)
        _plot_a_node_bipartite(verts, ax1, this_node_color)

    # plot the nodes for networks ----------
    for i_lbl, nodes_x in enumerate(nodes_x_start):
        verts = _build_node_verts_bipartite(nodes_x, 1.2, xw, yw)
        _plot_a_node_bipartite(verts, ax1, node_colors[i_lbl][:-1])
    ...
    plt.axis('off')
    # plot colorbar
    ax2 = plt.subplot2grid((8, 3), (7, 0))
    plot_colorbar2(edge_cmap_range, cmap_edge, ax2, ori='horizontal')
    ax2.title.set_text('edges color-code')

    ax3 = plt.subplot2grid((8, 3), (7, 1))
    plot_colorbar2(con_deg1, cmap_deg1, ax3, ori='horizontal')
    # plot_colorbar2(node_cmap_range, cmap_node, ax3, ori='horizontal')
    ax3.title.set_text('alpha nodes color-code')

    ax3 = plt.subplot2grid((8, 3), (7, 2))
    plot_colorbar2(con_deg2, cmap_deg2, ax3, ori='horizontal')
    ax3.title.set_text('nodes color-code')


def plot_connectivity_bipartite_2(con, labels, deg1, deg2, strength_perc, edge_cmp, node_cmap, fig_title='',
    ...
    # con_deg1 = np.sum(con_lbl_sorted, axis=1)
    # con_deg2 = np.sum(con_lbl_sorted, axis=0)
    con_deg1 = deg1[node_ind]
    con_deg2 = deg2[node_ind]
    node_cmap_range = np.arange(0, np.max(np.append(con_deg1, con_deg2)), 0.001)
    cmap_node = plt.get_cmap(node_cmap, len(node_cmap_range))
    cmap_node = cmap_node.reversed()
    # start x-coordinates ---------
    xw, yw = 1, 0.1
    nodes_x_start_lh = np.arange(0, len(lh_labels))
    ...
    # plot the nodes for degree ----------
    for i_lbl, nodes_x in enumerate(nodes_x_start):
        this_node_color = cmap_node(np.argmin(np.abs(node_cmap_range - con_deg1[i_lbl])))
        verts = _build_node_verts_bipartite(nodes_x, 1, xw, yw)
        _plot_a_node_bipartite(verts, ax1, this_node_color)

    for i_lbl, nodes_x in enumerate(nodes_x_start):
        this_node_color = cmap_node(np.argmin(np.abs(node_cmap_range - con_deg2[i_lbl])))
        verts = _build_node_verts_bipartite(nodes_x, -1, xw, yw)
        _plot_a_node_bipartite(verts, ax1, this_node_color)
...
def plot_connectivity_bipartite_3(con, labels, deg1, deg2, strength_perc, edge_cmp, node_cmap, fig_title='',
                                only_lbl=None, arrange='network'):
    import itertools
    from tools_general import plot_colorbar2
    ...
    n_lbl_lh = len(lh_labels)
    con_lbl_sorted = con[:, node_ind][node_ind, :]
    ind_triu = np.triu_indices(n_lbl)
    con_lbl_sorted[ind_triu] = 0
    if only_lbl is not None:
        con2 = np.zeros_like(con_lbl_sorted)
        con2[only_lbl, :] = con_lbl_sorted[only_lbl, :]
        con2[:, only_lbl] = con_lbl_sorted[:, only_lbl]
    ...
    plot_colorbar2(node_cmap_range, cmap_node, ax3, ori='horizontal')
    ax3.title.set_text('nodes color-code')
    return con_lbl_sorted, label_names
...
def plot_centrality_whole_brain(cent_ind, labels, src, subject, subjects_dir, colormap='Blues', clim='auto',
                                zscore=False, hemi='both', surface='pial', background='w'):
    from tools_source_space import label_idx_whole_brain, plot_stc_all_voxels
    if zscore:
        cent_ind = (cent_ind - np.mean(cent_ind)) / np.std(cent_ind)
    ...
    for label_no, label in enumerate(labels):
        ...
        stc_data[roi_idx, :] = cent_ind[label_no]
    # clim = dict(kind='value', pos_lims=[0, 0, max(stc_data)])
    return plot_stc_all_voxels(stc_data, src, subject, subjects_dir, clim=clim, colormap=colormap, smoothing=0,
                               hemi=hemi, surface=surface, background=background)
